...
@torch.no_grad()
def update_bn_stats(model, data_loader, hvd = None, num_iters = 200):
    """
    ...
    for ind, inputs in enumerate(itertools.islice(data_loader, num_iters)):
        logger.info('update_bn_stats: hvd.rank %s, step %s', hvd.rank(), ind)
        ...
        for i, bn in enumerate(bn_layers):
            ...
            if (torch.sum(torch.torch.isnan(bn.running_mean)) > 0
                or torch.sum(torch.torch.isnan(bn.running_var)) > 0):
                raise RuntimeError(
                    "update_bn_stats ERROR(hvd.rank {}): Got NaN val".format(hvd.rank()))
            if (torch.sum(torch.torch.isinf(bn.running_mean)) > 0
                or torch.sum(torch.torch.isinf(bn.running_var)) > 0):
                raise RuntimeError(
                    "update_bn_stats ERROR(hvd.rank {}): Got INF val".format(hvd.rank()))
            if (torch.sum(~torch.torch.isfinite(running_mean[i])) > 0
                or torch.sum(~torch.torch.isfinite(running_var[i])) > 0):
                raise RuntimeError(
                    "update_bn_stats ERROR(hvd.rank {}): Got NaN or INF val".format(hvd.rank()))
    ...
    for i, bn in enumerate(bn_layers):
        if hvd is not None and hvd.size() > 1:
            running_mean[i] = hvd.allreduce(running_mean[i])
            running_var[i] = hvd.allreduce(running_var[i])
