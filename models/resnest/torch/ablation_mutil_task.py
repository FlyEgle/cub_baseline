...
import torch
from .resnest_mutil_task import ResNet, Bottleneck
...
def resnest50_fast_2s1x64d(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(
            resnest_model_urls['resnest50_fast_2s1x64d'], progress=True))
    return model
...
def resnest50_fast_4s2x40d(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(
            resnest_model_urls['resnest50_fast_4s2x40d'], progress=True))
    return model
