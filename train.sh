...
# resnest200
cd /data/remote/yy_git_code/cub_baseline
output_ckpt="/data/remote/output_ckpt_with_logs/accv/ckpt/269_clean"
if [ ! -d "$output_ckpt" ]; then
  mkdir "$output_ckpt"
fi
horovodrun -np 8 -H localhost:8 python -W ignore train.py \
--batch-size=32 --num_thread=32 --fp16=1 --epochs=30 --warmup-epochs=0 --base-lr=0.0125 \
--num_classes=5000 --model_name="resnest269" --optimizer="sgd" --finetune=0 \
--pretrainmodel="imagenet" \
--imagenet_pretrain=1 --model_freeze=0 \
--log-dir="/data/remote/output_ckpt_with_logs/accv/logs/269_clean" \
--checkpoint-format="$output_ckpt" \
--train-dir="/data/remote/yy_git_code/cub_baseline/dataset/train_accv_clean_v3.log" \
--val-dir="" --cutmix=1 --cosine_lr=0 --size_type="resnet50_448" --use_focalloss=1 --labelSmooth=1 \
--resume_from_epoch=0 --wd=1e-5 
